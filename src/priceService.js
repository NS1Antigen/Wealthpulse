...
async function fetchXauUsd() {
  try {
    ...
    const possibleXau = goldData?.price ?? goldData?.ask ?? goldData?.bid ?? goldData?.data?.price ?? goldData?.rates?.XAU;
...
function calculateThaiGold1Baht(xauUsd, usdToThb) {
  ...
  const thaiGold1Baht = ((xauUsd * usdToThb * 15.244 * 0.965) / 31.1035) * THAI_GOLD_PREMIUM;
  if (!Number.isFinite(thaiGold1Baht) || thaiGold1Baht < 10000 || thaiGold1Baht > 200000) return null;
  ...
  for (const asset of assets) {
    try {
      if (asset.asset_type === "bitcoin") {
        ...
        prices[asset.ticker] = { price: data.bitcoin.usd, currency: "USD", source: "CoinGecko", change_24h_percent: data.bitcoin.usd_24h_change || 0 };
      } else if (asset.asset_type === "crypto_other") {
        const coinMap = { ETH: "ethereum", SOL: "solana", BNB: "binancecoin", XRP: "ripple", ADA: "cardano", DOGE: "dogecoin" };
        const coinId = coinMap[normalizeSymbol(asset.ticker)];
        if (coinId) {
          const res = await fetch(`https://api.coingecko.com/api/v3/simple/price?ids=${coinId}&vs_currencies=usd&include_24hr_change=true`, { cache: "no-store" });
          const data = await res.json();
          prices[asset.ticker] = { price: data[coinId].usd, currency: "USD", source: "CoinGecko", change_24h_percent: data[coinId].usd_24h_change || 0 };
        }
      } else if (asset.asset_type === "international_stock") {
        prices[asset.ticker] = { price: Number(asset.manual_value_thb) || 0, currency: "USD", source: "Manual Price" };
      } else if (asset.asset_type === "thai_stock") {
        prices[asset.ticker] = { price: Number(asset.manual_value_thb) || 0, currency: "THB", source: "Manual Price" };
        ...
        if (isScbSp500Fund(asset.ticker)) {
          ...
          prices[asset.ticker] = {
            ...
            currency: "THB",
            source: scbSp500Nav ? "Manual IVV Close × Realtime USDTHB" : "Manual NAV",
            manualIvvClose,
            usdToThb,
          ...
          prices[asset.ticker] = { price: Number(asset.manual_value_thb) || 0, currency: "THB", source: "Manual NAV" };
        }
      } else if (asset.asset_type === "thai_gold") {
        prices[asset.ticker] = {
          price: thaiGold1Baht || Number(asset.manual_value_thb) || 0,
          currency: "THB",
          source: thaiGold1Baht ? "Realtime Thai Gold Formula" : "Manual Thai Gold Price",
          xauUsd,
          usdToThb,
...
export function calculateAssetValue(asset, prices, usdToThb, currency = "THB") {
  ...
  if (asset.asset_type === "property" || asset.asset_type === "land" || asset.asset_type === "cash" || asset.asset_type === "other") {
    ...
    valueThb = priceData.currency === "USD" ? value * safeUsdToThb : value;
  }
...
export function calculateCostValue(asset, usdToThb, currency = "THB") {
  ...
  if (asset.purchase_currency === "USD") total *= safeUsdToThb;
